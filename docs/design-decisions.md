...
The selected User Type now supplies both a fixed hierarchy level and a permission subset. Separating immutable policy levels from configurable types prevents delegated privilege escalation. Two platform defaults cannot be deleted; other types require reassignment before soft deletion. Every vendor needs explicit recorded approval, including legacy active vendors lacking a decision. Agent/public vendor registration is deferred.
