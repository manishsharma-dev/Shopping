...
SessionUser contains id, name, email, and role. Role is customer, superadmin, admin, vendor, or vendor_admin. There is no frontend password-hash field or token-storage field.
...
Admin LoginPage calls login, checks admin/superadmin, logs out other roles, and navigates to /dashboard. Admin's route guard restores before allowing protected navigation. The admin App calls logout and navigates to /login after success.
