...
User/type/vendor creation now uses Material form fields, selects, inputs and permission checkboxes. Native product controls are retained. A shared AccessService drives both sidebar visibility and guarded routing from current backend capabilities; hiding links alone is never authorization.
