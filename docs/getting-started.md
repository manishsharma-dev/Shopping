...
Sign in as superadmin, create shared categories and User Types, create a vendor (or approve an application), then create its vendor_admin account. Select a lower User Type in Users, then choose its required state/district from imported reference data. A vendor administrator can create custom fields, products and staff using shared or vendor-local types. Agents can be created by scoped superiors to submit vendors. Public vendor/Agent registration is deferred; admin-created vendors also require approval. Manual order capture and metrics are available in admin; customer checkout/payment remain unimplemented. See [management](backend/features/management.md).

On restart, the management module initializes default User Type records without creating accounts or resetting passwords. Older active vendors without a recorded approval must be reviewed before business access resumes. See [hierarchy onboarding](backend/features/user-types.md).
